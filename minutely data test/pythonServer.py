...
# conn = sqlite3.connect('databases\sensor_data.db') #windows
conn = sqlite3.connect('databases/sensor_data.db') #linux
cursor = conn.cursor()
hostname = "localhost"
port = 5000
...
def fetch_data_between_timestamps(start_timestamp, end_timestamp):
    query = 'SELECT * FROM sensor_data_minutely WHERE minute_start >= ? AND minute_start <= ?'
